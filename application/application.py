...
def application_txd(node, node_type, start_flag, my_system_rxd_queue, ca_service_txd_queue, den_service_txd_queue, au_info, obu_info, rsu_info):
    ...
    while True:
        i = i+1
        den_user_data = trigger_event(
            node, node_type, au_info, obu_info, rsu_info)
...
def my_system(node, node_type, start_flag, coordinates, obd_2_interface, my_system_rxd_queue, den_service_txd_queue, movement_control_txd_queue, au_info, obu_info, rsu_info, dest, position_rxd_queue):
    ...
    if (obu_info != {}):
        obu_temp = update_obu_info(
            obu_temp, obu_info['name'], obu_info['destination'], obu_info['max_capacity'], obu_info['free'])
    ...
    while True:
        ...
        if msg_rxd['msg_type'] == 'DEN':
            ...
            if node_type == 'RSU' and msg_rxd['event']['receiver_node_type'] == 'RSU':
                ...
                if (msg_rxd['event']['sender_node_type'] == 'OBU' and msg_rxd['event']['msg_type'] == 'booking'):
                    ...
                    type = 'AU'
                    den_user_data = {'node': node, 'sender_node_type': node_type,
                                     'receiver_node_type': type, 'obu_name': obu_temp['name'], 'confirmation': confirmation}
                    den_service_txd_queue.put(den_user_data)
                ...
                if (msg_rxd['event']['sender_node_type'] == 'OBU' and msg_rxd['event']['msg_type'] == 'entered'):
                    ...
                    obu_temp = update_obu_info(
                        obu_temp, obu_temp['name'], obu_temp['destination'], obu_temp['max_capacity'], msg_rxd['event']['free space'])
                    rsu_temp = update_rsu_info(rsu_temp['id'], obu_temp)
                    ...
                    type = 'OBU'
                    den_user_data = {'node': node, 'sender_node_type': node_type,
                                     'receiver_node_type': type, 'free space': obu_temp['free'], 'msg_type': 'entered'}
                    den_service_txd_queue.put(den_user_data)
                ...
                if (msg_rxd['event']['sender_node_type'] == 'OBU' and msg_rxd['event']['msg_type'] == 'left'):
                    ...
                    type = 'OBU'
                    den_user_data = {'node': node, 'sender_node_type': node_type,
                                     'receiver_node_type': type, 'free space': obu_temp['free'], 'msg_type': 'left'}
                    den_service_txd_queue.put(den_user_data)
            ...
            if node_type == 'AU' and msg_rxd['event']['receiver_node_type'] == 'AU':
                ...
                if (msg_rxd['event']['sender_node_type'] == 'RSU'):
                    ...
                    if (msg_rxd['event']['confirmation'] == 'NOT OK'):
                        print(
                            'Trip canceled, no seats available or no OBU going to your destination :(')
            ...
            if node_type == 'OBU' and msg_rxd['event']['receiver_node_type'] == 'OBU':
                ...
                if (msg_rxd['event']['sender_node_type'] == 'RSU' and msg_rxd['event']['msg_type'] == 'booking'):
                    ...
                    # booking NOT OK
                    res = int(obu_temp['free']) - \
                        int(msg_rxd['event']['num_passengers'])
        ...
        if (node_type == 'OBU'):
            obu_info = update_obu_info(
                obu_temp, obu_temp['name'], obu_temp['destination'], obu_temp['max_capacity'], obu_temp['free'])
